...
public class ProctorService(IMapper mapper, IClock time) : IProctorService
{
    ...
    public async Task<ExamContentOutput> GenerateExamContent(GenerateExamContentInput input)
    {
        ...
        var testRun = await DB.Find<TestRun>().MatchID(exam.TestRunId).ExecuteSingleAsync();
    ...
    public async Task<FinishExamOutput> FinishExam(FinishExamInput input)
    {
        ...
        exam.Grading = testDefinition.GradingSettings.CalculateGrading(exam.FinalMark,
            selectedQuestions.Sum(c => c.ScoreSettings.TotalPoints));
    ...
    public async Task<QuestionDefinition?> GetTestRunQuestion(string examId, string questionId)
    {
        ...
        var batches = await DB.Find<TestRunQuestion>()
            .ManyAsync(c => c.TestRunId == exam.TestRunId && c.Batch.Any(i => i.ID == questionId));
    ...
    {
        return await DB.Find<Exam>().Match(c => c.TestRunId == testRunId && c.AccessCode == accessCode)
            .ExecuteSingleAsync();
    }
    ...
    {
        return await DB.Find<TestDefinition>().MatchID(testDefinitionId).ExecuteSingleAsync() ??
               throw new TestPlatformException("NotFoundTestDefinition");
    }

    private static Dictionary<string, int> CalculateQuestionScores(List<QuestionDefinition> questions,
    ...
    {
        var key = ((int)InformFactor.CorrectAnswers).ToString();

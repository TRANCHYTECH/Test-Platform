...
public class TimeSettingsPartValidator : AbstractValidator<TimeSettingsPart>
{
    ...
    {
        context.RootContextData["TimeSettingsPart.TestDurationMethod"] =
            context.InstanceToValidate.TestDurationMethod;
...
public class AnswerQuestionConfigValidator : AbstractValidator<AnswerQuestionConfig>
{
    public AnswerQuestionConfigValidator()
    {
        ...
        {
            var testDurationMethod =
                ctx.RootContextData["TimeSettingsPart.TestDurationMethod"] as TestDurationMethod;
            if (testDurationMethod is CompleteQuestionDuration && field)
            {
                return false;
            }
            ...
            "ERR.TESTDEF.TIME.001"); // When test duration method is CompleteQuestionDuration, not allow to skip question.
    }
}
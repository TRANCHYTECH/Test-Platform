...
{
    public static void AssertError<T>(this TestValidationResult<T> validationResult, string forPropertyName, string withErrorMessage)
    {
        Assert.Equal(1, validationResult.Errors.Count(c => c.PropertyName == forPropertyName && c.ErrorCode == withErrorMessage));
    }
}
...
public class QuestionDefinitionValidatorUnitTests
{
    ...
    public async Task QuestionDefinition_Validate_Failure_Business_Logics()
    {
        var input = new QuestionDefinition()
        {
            ...
            Description = Faker.Lorem.Paragraph(3000),

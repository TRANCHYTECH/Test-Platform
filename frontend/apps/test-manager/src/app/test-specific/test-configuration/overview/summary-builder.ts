...
import { injectPortalConfig, UserProfileService } from '@viet-geeks/core';
import {
  TestCategory,
  TestCategoryUncategorizedId,
} from '../../../_state/test-category.model';
import { keys, sumBy } from 'lodash-es';
import { QuestionSummary } from '@viet-geeks/shared';
import {
  BasicSettings,
...
import {
  GradingCriteriaConfigType,
  RangeUnit,
} from '../../_state/ui/grading-summary-ui.model';
import { TestConfigRoutes } from '../test-configuration-routing.module';
import {
  TestActivationMethodType,
  TestDurationMethod,
} from '../test-time-settings/test-time-settings.component';
...
export class SummaryBuilder {
  ...
  private _appSettingsService = injectPortalConfig<PortalConfig>();
  private _userProfileService = inject(UserProfileService);

  buildSummaryForDaftTest(
    test: Test,
    testCategories: TestCategory[],
    questionSummary: QuestionSummary[]
  ) {
    return [
      this.getBasicSettingSummaryForDraftTest(
        test.basicSettings,
        testCategories
      ),
  ...
  private getTimeSummaryForDraftTest(part: TimeSettings): Summary {
    ...
    if (part.testDurationMethod.$type === TestDurationMethod.CompleteTestTime) {
      const completeTestDuration =
        part.testDurationMethod as CompleteTestDuration;
      testDuration = this._translate.instant('summary.testTimeLimit', {
        duration: completeTestDuration.duration,
      });
    } else if (
      part.testDurationMethod.$type === TestDurationMethod.CompleteQuestionTime
    ) {
      const completeQuestionDuration =
        part.testDurationMethod as CompleteQuestionDuration;
      testDuration = this._translate.instant('summary.questionTimeLimit', {
        duration: completeQuestionDuration.duration,
      });
    }
    ...
    switch (part.testActivationMethod.$type) {
      ...
      case TestActivationMethodType.TimePeriod: {
        ...
        return {
          url: this.getConfigRouteLink(TestConfigRoutes.TimeSettings),
          status: 'ok',
          desc: `${this._translate.instant('summary.timePeriodActivation', {
            activeFromDate:
              this._userProfileService.convertUtcToLocalDateString(
                timePeriod.activeFromDate
              ),
            timeZone: this._userProfileService.currentTimeZone,
          })}. ${testDuration}.`,
        };
      }
  ...
  private getGradingSummaryForDaftTest(part: GradingSettings): Summary {
    if (keys(part.gradingCriterias).length === 0) {
      ...
      const passMarkGrade = part.gradingCriterias[
        GradingCriteriaConfigType.PassMask
      ] as PassMaskCriteria;
      const rangesGrade = part.gradingCriterias[
        GradingCriteriaConfigType.GradeRanges
      ] as GradeRangeCriteria;
      const passMarkMsg =
        passMarkGrade !== undefined
          ? `Test pass mark: ${passMarkGrade.value} ${
              RangeUnit.Percent === passMarkGrade.unit ? 'Percent' : 'Point'
            }. `
          : '';
      const gradeRangesMsg =
        rangesGrade != undefined
          ? `Grading is based on ${rangesGrade.details.length} ${
              RangeUnit.Percent === rangesGrade.unit ? 'Percent' : 'Point'
            } range`
          : '';
      return {
        url: this.getConfigRouteLink(TestConfigRoutes.GradingAndSummary),
  ...
  private getQuestionSummaryForDraftTest(
    questionSummary: QuestionSummary[]
  ): Summary {
  ...
  private getBasicSettingSummaryForDraftTest(
    part: BasicSettings,
    testCategories: TestCategory[]
  ): Summary {
    if (part.category === TestCategoryUncategorizedId) {
      ...
      return {
        url: this.getConfigRouteLink(TestConfigRoutes.BasicSettings),
        status: 'ok',
        desc: `Your test belongs to the category ${
          testCategories.find((c) => c.id === part.category)?.name
        }.`,
      };
    }
  }
  ...
  private getTestTimeUtc(config: TestActivationMethod): {
    startAt?: Date;
    endAt: Date;
  } {
    switch (config.$type) {
      ...
      case TestActivationMethodType.TimePeriod: {
        ...
        return {
          startAt: method.activeFromDate,
          endAt: method.activeUntilDate,
        };
      }
      default:
  ...
  private getTimeSummaryForActiveTest(part: TimeSettings): Summary {
    ...
    return {
      status: 'ok',
      desc: `Test access will close on: ${this._userProfileService.convertUtcToLocalDateString(
        testTime.endAt
      )} (${
        this._userProfileService.currentTimeZone
      }). Current server time is: ${
        this._userProfileService.currentUserTime
      } (${this._userProfileService.currentTimeZone}).`,
    };
  }

  private getTestAccessSummaryForActiveTest(
    part: TestAccess,
    testId: string
  ): Summary {
    switch (part.settings.$type) {
      case TestAccessType.PublicLink:
        return {
          desc: 'Test access: PublicLink',
          status: 'ok',
          url: `${this._appSettingsService.testRunnerBaseUrl}/test/${testId}`,
        };
      case TestAccessType.PrivateAccessCode:

...
export class TestsService {
  ...
  get() {
    return this._http.get<Test[]>(`${this.testManagerApiBaseUrl}/Management/QuestionDefinition`).pipe(tap(entities => {
  ...
  getById(id: string) {
    ...
    return this._http.get<Test>(`${this.testManagerApiBaseUrl}/Management/QuestionDefinition/${id}`).pipe(
      tap(rs => {
  ...
  add(test: Partial<Test>) {
    return this._http.post<Test>(`${this.testManagerApiBaseUrl}/Management/QuestionDefinition`, test).pipe(tap(rs => {
      this._testsStore.add(rs);
    }));
  }

  update(id: string, test: Partial<Test>) {
    return firstValueFrom(this._http.put<Test>(`${this.testManagerApiBaseUrl}/Management/QuestionDefinition/${id}`, test).pipe(tap(rs => {
